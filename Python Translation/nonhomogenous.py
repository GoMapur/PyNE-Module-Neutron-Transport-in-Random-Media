def non_homogenous(var):
    ...
    for t in arange(1, N / 2).reshape(-1):
        s = (t - 1) * n1
        A[s, s] = - u[t - 1] * (1 / h[0] + 1 / (h[0] + h[1])) + \
            Et12[L[0, 1] - 1] - Es12[L[0, 1] - 1] * wt[t - 1] / 2
        ...
        if l == 1 and N > 2:
            for p in arange(l + 1, N / 2).reshape(-1):
                ...
                for i in arange(1, n1).reshape(-1):
                    if L[i - 1, 1] == 3:
                        A[s + i - 1, S + i - 1] = - \
                            Es12[L[i, 1] - 1] * wt[p - 1] / 2
                    else:
                        A[s + i - 1, S + i - 1] = - \
                            Es12[L[i - 1, 1] - 1] * wt[p - 1] / 2
        else:
            if l > 1 and N > 2:
        ...
        for p in arange(1, N / 2).reshape(-1):
            ...
            for i in arange(2, n1).reshape(-1):
                if L[i - 1, 1] == 3:
                    A[s + i - 1, S + i - 0] = - \
                        Es12[L[i, 1] - 1] * wt[N / 2 + p - 1] / 2
                else:
                    A[s + i - 1, S + i - 0] = - \
                        Es12[L[i - 1, 1] - 1] * wt[N / 2 + p - 1] / 2
            a = a + (Es12[L[0, 1] - 1] * wt[N / 2 + p - 1] * yo / 2)
        B[s] = B[s] + a
    for t in arange(N / 2 + 1, N).reshape(-1):
        s = (t - 1) * n1
        A[s, s] = u[t - 1] * (h[1] - h[0]) / (h[1] * h[0]) + \
            Et12[L[0, 1] - 1] - Es12[L[0, 1] - 1] * wt[t - 1] / 2
        ...
        A[s + n1 - 1, s + n1 - 0] = - u[t - 1] * \
            (1 / h[n1 - 1] + 1 / h[n1 - 0])
        A[s + n1 - 1, s + n1 - 3] = u[t - 1] * \
            (h[n1 - 1] / (h[n1 - 0] * (h[n1 - 1] + h[n1 - 0])))
        B[s + n1 - 1] = 0.0
        l = copy(t)
        if l == N / 2 + 1 and N > 2:
